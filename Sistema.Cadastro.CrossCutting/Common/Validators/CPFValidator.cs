namespace Sistema.Cadastro.CrossCutting.Common.Validators
{
    ...
    {
        private static readonly HashSet<string> InvalidDocs = new HashSet<string> { "00000000000", "22222222222", "33333333333", "44444444444", "55555555555", "66666666666", "77777777777", "88888888888", "99999999999" };

        public static bool Validar(string numero)
        {
            numero = new string(numero.Where(char.IsDigit).ToArray());

            if (numero.Length != 11 || InvalidDocs.Contains(numero))
                return false;
